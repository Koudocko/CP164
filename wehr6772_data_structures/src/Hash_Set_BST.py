...
class Hash_Set_BST:
    ...
    def __contains__(self, key):
        ...
        """
        # your code here
        slot_list = self._table[hash(key) % self._capacity]
        contains = slot_list.__contains__(key)
    ...
    def insert(self, value):
        ...
        if value not in slot_list:
            slot_list.insert(deepcopy(value))
            self._count += 1
    ...
    def _rehash(self):
        ...
        while len(table_ref) > 0:
            ...
            while not slot_ref.is_empty():
                val = slot_ref.remove(slot_ref._root._value)
                self._table[hash(val) % self._capacity].insert(val)
    ...
    def debug(self):
        ...
        for idx in range(0, self._capacity):
            print("----------------------------------------")
            print(f"Slot {idx}\n")

            for val in self._table[idx]:
                print(val)
