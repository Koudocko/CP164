...
def fill_letter_bst(bst, values):
	...
	# Your code here
    for val in values:
        bst.insert(ByLetter(val[0], val[1]))
...
def encode_morse(bst, text):
    ...
    for char in text:
        if char.isalpha():
            result += bst.retrieve(ByLetter(char.upper(), None)).code + " "
        else:
            result += "\n"
...
def decode_morse(bst, code):
    ...
    for word in code.split('\n'):
        for char in word.split():
            result += bst.retrieve(ByCode(None, char)).letter
        result += " "
