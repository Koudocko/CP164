...
class BST:
    ...
    def _are_flipped_aux(self, node, node_flip):
        """
        -------------------------------------------------------
        ...
        if node is not None and node_flip is not None:
            flipped = node._value == node_flip._value

            if flipped:
                flipped = self._are_flipped_aux(node._left, node_flip._right)

                if flipped:
                    flipped = self._are_flipped_aux(node._right, node_flip._left)
        elif node == node_flip:
            flipped = True
    ...
    def _total_height_aux(self, node):
        ...
        if node is not None:
            total_height = node._height + \
                self._total_height_aux(node._left) + \
    ...
    def furthest(self):
        ...
        # Your code here
        left, right = self._root, self._root
        values = []
