...
class Deque:
    ...
    def is_mirror(self):
        ...
        half = Deque()
        half._front = half._rear = self._front

        while middle > 0 and half._rear is not None:
            half._rear = half._rear._next
            middle -= 1

        currA, currB = half._front, self._rear

        while mirror and currA is not None and currB is not None:
            mirror = currA._value == currB._value
